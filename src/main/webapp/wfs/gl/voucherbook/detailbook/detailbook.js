Ext.namespace("wfs.gl.voucherbook.detailbook");

wfs.gl.voucherbook.detailbook.QueryFormPanel = Ext.extend(Ext.FormPanel, 
{
	frame : true,
	labelWidth : 100,
	labelAlign : "right",
	...
	initComponent : function() 
	{
    	...
    	this.startAccount = new gl.component.xychooseaccount(
		{
    		...
    		codecondition : '0,1,2,3,4,7,8,9,10',
    		fieldLabel : '科目从'
		});
    	...
    	//调用构造器
    	wfs.gl.voucherbook.detailbook.QueryFormPanel.superclass.initComponent.call(this);
	...
	query : function() 
	{
		var beginaccount = this.startAccount.getCodeValue();	//开始科目code
		...
		Ext.Ajax.request(
		{
			url : "wfs/voucherbook/detailbook/gettableName",
			params :
			{
			...
			success : function(response)
			{
				...
				if (r.success) 
				{
					...
					this.xy_MainPanel.QueryPageEvent.call(this.xy_MainPanel,
					{
						 ...
						 */
						jasper : "/wfs/gl/voucherbook/detailbook/detailbook_see.jrxml",
						/**
						 * (导出)报表jasper文件路径(非url)，从webapp/webRoot目录算起
						 */
						jasper_export : "/wfs/gl/voucherbook/detailbook/detailbook.jrxml",
						/**
						 * 报表标题，必填
						 */
						title : "明细账",
						 ...
						 * 报表查询参数
						 */
						jsonCondition : Ext.encode(tablename)
					},
					/**
					 * 报表查询url，与Action的@RequestMapping保持一致
					 */
					"/wfs/voucherbook/detailbook");
				}
				else
				{
...
function init() 
{
	...
	{
		xy_QueryPanel : wfs.gl.voucherbook.detailbook.QueryFormPanel,
