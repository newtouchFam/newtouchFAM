...
gl.vouchermanager.vouchercheck.vouchercheckcondition = Ext.extend(Ext.FormPanel,
{
	frame : false,
	height : 112,
    border : false,
    isView : false,
    vouchercheckdetail : null,
    getRecord : function()
    {
    ...
    getStore : function()
    {
        if (this.store === undefined || this.store == null)
        {
    		...
    		{
                url : 'vouchermanager/vouchermain/initcheck',
    ...
    var row3 =
    	...
    	this.checkbutton = new Ext.Button(
        	{
        		...
        		text :'审核',
        		iconCls : "xy-act-post ",
    			handler :this.checkHandler.createDelegate(this),
    			minWidth:80
        	});
        	
    	this.uncheckbutton = new Ext.Button(
        	{
        		id : "uncheckbutton",
        		text :'反审核',
        		iconCls : "xy-stop ",
    			handler : this.uncheckHandler.createDelegate(this),
    			minWidth:80
        	});
        	
		var barTop = ['审核日期：', this.checkdate, '-', this.checkbutton, '-', this.uncheckbutton];
		this.bbar = barTop;
    	...
    	gl.vouchermanager.vouchercheck.vouchercheckcondition.superclass.initComponent.call(this);
	...
	queryHandler : function ()
	{
		...
		obj.checkdate = Ext.getCmp("checkdate").getValue();
		obj.intstatus = Ext.getCmp("intstatus").getXyValue();
		this.vouchercheckdetail.getStore().baseParams.jsonCondition = Ext.encode(obj);
		this.vouchercheckdetail.getStore().load(
		{
			params : 
			{
				start : 0,
	...
	{
		var records = this.vouchercheckdetail.getSelectionModel().getSelections();
	...
	checkHandler : function ()
	{
		...
		if(Ext.getCmp("checkdate").getValue() == "" || Ext.getCmp("checkdate").getValue() == null)
		{
			Ext.Msg.alert("提示","请选择审核日期!");
			return;
		}
		...
		for(var i = 0; i < records.length; i ++)
		{
			...
			if(records[i].get('intstatus') == '已审核')
			{
				Ext.Msg.alert("提示","已审核的凭证不能审核!");
				return;
			}
		}
		...
		Ext.Ajax.request(
		{
			url : "vouchermanager/vouchermain/check",
			params :
			{
				vouchercheckdate : Ext.getCmp("checkdate").getValue().format('Y-m-d'),
			...
			success : function(response)
			{
				...
				if (r.success)
				{
					Ext.Msg.alert("提示", "凭证审核成功");
					this.vouchercheckdetail.getStore().reload();
				}
				else
				{
	...
	uncheckHandler : function ()
	{
		...
		Ext.Ajax.request(
		{
			url : "vouchermanager/vouchermain/uncheck",
			params :
			{
