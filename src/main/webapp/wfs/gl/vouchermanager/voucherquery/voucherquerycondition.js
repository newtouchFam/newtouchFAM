...
gl.vouchermanager.voucherquery.voucherquerycondition = Ext.extend(Ext.FormPanel,
{
	frame : false,
	height : 88,
    border : false,
    voucherquerydetail : null,
    getRecord : function()
    {
    ...
    getStore : function()
    {
        if (this.store === undefined || this.store == null)
        {
    		...
    		{
                url : 'vouchermanager/vouchermain/initquery',
    ...
    initComponent : function ()
	{
    	...
    	this.printAllVoucherButton = new Ext.Button(
    	{
    		id : "prinalltvoucherbutton",
    		text : "打印全部",
			handler : this.printAllVoucherHandler.createDelegate(this),
			minWidth:70
    	});
	    ...
	    var row2 =
	    {
			...
			},
			{
				...
				items : [ this.printAllVoucherButton ]
			}]
		};
    	
    	this.items = [row0, row1, row2];
    	var barTop = ['-',this.editbutton,'-', this.deletebutton ];
    	
    	this.getStore().on("load", this.loaded, this);
		
    	gl.vouchermanager.voucherquery.voucherquerycondition.superclass.initComponent.call(this);
	...
	{
		var records = this.voucherquerydetail.getSelectionModel().getSelections();
				...
				Ext.Ajax.request(
				{
					...
					success : function(response)
					{
						...
						if (r.success)
						{
							...
							this.voucherquerydetail.getStore().reload();
						}
						else
						{
	...
	queryHandler : function ()
	{
		...
		this.voucherquerydetail.getStore().baseParams.jsonCondition = Ext.encode(obj);
		this.voucherquerydetail.getStore().load(
		{
			params : 
			{
				start : 0,
	...
	printVoucherHandler : function()
	{
		...
		else
		{
			...
			VoucherPrintUtil.voucher_printsPDF(voucherids);
		}
	},
	printAllVoucherHandler : function()
	{
		var detailData = new Array();
		var count = this.voucherquerydetail.getStore().getCount();
		if(count <= 0)
		{
			Ext.Msg.alert("提示", "没有凭证记录！");
			return;
		}
		for(var i = 0 ; i < count; i++)
		{
			var record = this.voucherquerydetail.getStore().getAt(i);
			var voucherid=record.get('voucherid');
			var obj = 
			{
				voucherid : voucherid
			};
