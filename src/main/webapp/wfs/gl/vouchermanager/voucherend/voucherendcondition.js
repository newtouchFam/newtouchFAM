...
gl.vouchermanager.voucherend.voucherendcondition = Ext.extend(Ext.FormPanel,
{
	frame : false,
	height : 112,
    border : false,
    voucherenddetail : null,
    getRecord : function()
    {
    ...
    getStore : function()
    {
        if (this.store === undefined || this.store == null)
        {
    		...
    		{
                url : 'vouchermanager/vouchermain/initend',
   ...
   /* 	var row1 = 
		{
            ...
            items : [{
                        ...
                        border : false,
                        layout : 'fit',
                        items : [this.dtfilldatefromlabel]
                    }, {
                        columnWidth : .13,
                        border : false,
                        layout : 'fit',
    	...
    	this.blank1 = new Ext.form.Field
    			({
    	              id:"blank1",
    	              text:""	  
    	        });
    	...
    	/*this.endbutton = new Ext.Button(
    	{
    		id : "endbutton",
    		text :'记账',
			handler :this.endHandler.createDelegate(this)
    	});*/
    ...
    var row3 =
    	...
    	var barTop = ['记账日期：',this.enddate,'-', endbutton ];
		this.bbar = barTop;

    	this.getStore().on("load", this.loaded, this);
		
    	gl.vouchermanager.voucherend.voucherendcondition.superclass.initComponent.call(this);
	...
	queryHandler : function ()
	{
		...
		this.voucherenddetail.getStore().baseParams.jsonCondition = Ext.encode(obj);
		this.voucherenddetail.getStore().load(
		{
			params : 
			{
				start : 0,
	...
	{
		var records = this.voucherenddetail.getSelectionModel().getSelections();
	...
	endHandler : function ()
	{
		...
		if(Ext.getCmp("enddate").getValue() == "" || Ext.getCmp("enddate").getValue() == null)
		{
			Ext.Msg.alert("提示","请选择记账日期!");
			return;
		}
		...
		Ext.Ajax.request(
		{
			url : "vouchermanager/vouchermain/end",
			params :
			{
				voucherenddate : Ext.getCmp("enddate").getValue().format('Y-m-d'),
			...
			success : function(response)
			{
				...
				if (r.success)
				{
					Ext.Msg.alert("提示", "凭证记账成功");
					this.voucherenddetail.getStore().reload();
				}
				else
				{
