 ...
 * 表单组件属性设置工具类
 */
ssc.smcs.common.FormFieldAttrUtil = {};

FormFieldAttrUtil = ssc.smcs.common.FormFieldAttrUtil;
 ...
 */
ssc.smcs.common.FormFieldAttrUtil.setFormAttr = function(/* jsonString */ fieldAttrConfig, /* array<string> */ arrayGridPanelID)
{
	...
	var config = new ssc.smcs.common.FormFieldAttrConfig(fieldAttrConfig);
	...
	for (var i = 0; i < config.getFieldIDList().length; i++)
	{
		...
		ssc.smcs.common.FormFieldAttrUtil.setFieldAttr(config, strFieldID);
	}
	...
	for (var i = 0; i < config.getGridPanelIDList().length; i++)
	{
		...
		ssc.smcs.common.FormFieldAttrUtil.setGridAttr(config, strGridPanelID);
	}
};
 ...
 * 计算表头或列头文本，包括必填标志
 */
ssc.smcs.common.FormFieldAttrUtil.getLabelByKey = function(label, key)
{
 ...
 * 设置表头字段显示必填样式
 */
ssc.smcs.common.FormFieldAttrUtil.setFieldLabelKey = function(field, key)
{
	...
	var label = ssc.smcs.common.FormFieldAttrUtil.getLabelByKey(field.fieldLabel, key);
 ...
 * strFieldID				组件ID
 */
ssc.smcs.common.FormFieldAttrUtil.setFieldAttr = function(config, strFieldID)
{
	...
	{
		field.fieldLabel = ssc.smcs.common.FormFieldAttrUtil.getLabelByKey(fieldLabelAttr, config.getFieldKey(strFieldID));
...
/*		if (config.getFieldKey(strFieldID))
	...
	if (fieldAlignAttr == ssc.smcs.common.FieldAlignEnum.Left)
	{
		field.style = "text-align:left;";
	}
	else if (fieldAlignAttr == ssc.smcs.common.FieldAlignEnum.Center)
	{
		field.style = "text-align:center;";
	}
	else if (fieldAlignAttr == ssc.smcs.common.FieldAlignEnum.Right)
	{
	...
	if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Text)
	{
		/* 文本类型，不处理 */
	}
	else if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Date)
	{
		/* 日期字段，标准日期格式，靠左对齐 */
		field.style = "text-align:left";
		field.format = "Y-m-d";
	...
	else if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Time)
	{
		/* 时间字段，标准时间格式，靠左对齐 */
		field.style = "text-align:left";
		field.format = "H:i:s";
	...
	else if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Integer)
	{
		/* 整数字段，靠右对齐，无小数位 */
		field.style = "text-align:right";
		field.allowDecimals = false;
	}
	else if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Float)
	{
		/* 小数字段，靠右对齐，有小数位 */
		field.style = "text-align:right";
		field.allowDecimals = true;
	}
	else if (fieldTypeAttr == ssc.smcs.common.FieldTypeEnum.Money)
	{
 ...
 * strGridPanelID	表格ID
 */
ssc.smcs.common.FormFieldAttrUtil.setGridAttr = function(config, strGridPanelID)
{
	...
	for (var i = 0; i < config.getColumnDataIndexList(strGridPanelID).length; i++)
	{
		...
		ssc.smcs.common.FormFieldAttrUtil.setColumnAttr(config, gridPanel, strColumnDataIndex);
	}
};
 ...
 * 设置表格列头文字
 */
ssc.smcs.common.FormFieldAttrUtil.setColumnHeader = function(gridPanel, strDataIndex, header, key)
{
	...
	{
		label = ssc.smcs.common.FormFieldAttrUtil.getLabelByKey(header, key);
	}
	else
	{
		label = header;
 ...
 * 设置表格列头显示必填样式
 */
ssc.smcs.common.FormFieldAttrUtil.setColumnHeaderKey = function(gridPanel, strDataIndex, key)
{
	if (gridPanel == null || gridPanel == undefined || gridPanel.getColumnModel == undefined)
	{
		return;
	...
	var label = ssc.smcs.common.FormFieldAttrUtil.getLabelByKey(columnModel.getColumnHeader(intColumnIndex), key);
 ...
 * strDataIndex		表格列索引
 */
ssc.smcs.common.FormFieldAttrUtil.setColumnAttr = function(config, gridPanel, strDataIndex)
{
	var columnModel = gridPanel.getColumnModel();
	...
	if (columnAlignAttr == ssc.smcs.common.FieldAlignEnum.Left)
	{
		columnModel.config[intColumnIndex].css = "text-align:left;";
	}
	else if (columnAlignAttr == ssc.smcs.common.FieldAlignEnum.Center)
	{
		columnModel.config[intColumnIndex].css = "text-align:center;";
	}
	else if (columnAlignAttr == ssc.smcs.common.FieldAlignEnum.Right)
	{
	...
	if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Text)
	{
		/* 文本类型，不处理 */
	}
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Date)
	{
		/* 日期字段，标准日期格式，居中 */
		columnModel.config[intColumnIndex].css = "text-align:center;";
	...
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Time)
	{
		/* 时间字段，标准时间格式，居中 */
		columnModel.config[intColumnIndex].css = "text-align:center;";
		if (cellEditor != null && cellEditor.field != null && cellEditor.field != undefined)
	...
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Integer)
	{
		/* 整数字段，靠右对齐，无小数位 */
		columnModel.config[intColumnIndex].css = "text-align:right;";
	}
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Float)
	{
		/* 小数字段，靠右对齐，有小数位 */
		columnModel.config[intColumnIndex].css = "text-align:right;";
	}
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Money)
	{
		/* 金额字段，靠右对齐，标准金额格式(千分位、2位小数) */
		columnModel.config[intColumnIndex].css = "text-align:right;";
	...
	else if (columnTypeAttr == ssc.smcs.common.FieldTypeEnum.Percent)
	{
		/* 百分比(千分位、2位小数、百分号) */
		columnModel.config[intColumnIndex].css = "text-align:right;";
	...
	if (columnSumTypeAttr == ssc.smcs.common.SummaryTypeEnum.None)
	{
 ...
 * fieldAttrConfig	表单组件通用属性配置
 */
ssc.smcs.common.FormFieldAttrUtil.setXyCalcRecordAttr = function(/* jsonString */ fieldAttrConfig, strGridPanelID, /* Ext.data.XyCalcRecord */record)
{
	if (fieldAttrConfig == undefined || fieldAttrConfig == null)
	{
		/* 断言 */
	...
	for (var i = 0; i < config.getColumnDataIndexList(strGridPanelID).length; i++)
	{
		...
		ssc.smcs.common.FormFieldAttrUtil.setXyCalcRecordFieldAttr(config, strGridPanelID, strColumnDataIndex, record);
	}
};
 ...
 * strGridPanelID	表格ID
 */
ssc.smcs.common.FormFieldAttrUtil.setXyCalcRecordFieldAttr = function(config, strGridPanelID, strDataIndex, /* Ext.data.XyCalcRecord */record)
{
