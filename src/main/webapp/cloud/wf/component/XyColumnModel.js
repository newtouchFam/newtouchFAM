...
Ext.grid.XyColumnModel = function(A) {
	...
	Ext.grid.XyColumnModel.superclass.constructor.call(this)
};
Ext.extend(Ext.grid.XyColumnModel, Ext.util.Observable, {
	...
	setConfig : function(C, B) {
		...
		for (var D = 0, A = C.length; D < A; D++) {
			...
			if (E.editor && E.editor.isFormField) {
				E.editor = new Ext.grid.XyGridEditor(E.editor)
			}
			this.lookup[E.id] = E
		}
		if (!B) {
	...
	getRenderer : function(A) {
		if (!this.config[A].renderer) {
			return Ext.grid.XyColumnModel.defaultRenderer
		}
...
Ext.grid.XyColumnModel.defaultRenderer = function(A) {
...
Ext.grid.DefaultColumnModel = Ext.grid.XyColumnModel;