...
Ext.grid.XyGridView = function(A) {
	Ext.apply(this, A);
	this.addEvents("beforerowremoved", "beforerowsinserted", "beforerefresh",
			"rowremoved", "rowsinserted", "rowupdated", "refresh");
	Ext.grid.XyGridView.superclass.constructor.call(this)
};
Ext.extend(Ext.grid.XyGridView, Ext.util.Observable, {
	scrollOffset : 19,
	autoFill : false,
	forceFit : false,
	...
	doRender : function(E, G, M, A, L, Q) {
		...
		for (var P = 0, S = G.length; P < S; P++) {
			...
			for (var R = 0; R < L; R++) {
				...
				try {
					if (typeof(K.data[U.name]) == "string" && K.data[U.name].charAt(0) == '{') 
					{
						var obj = JSON.parse(K.data[U.name]);
						if (K.displayData[U.name] === undefined) {
							K.displayData[U.name] = obj[this.grid.colModel.config[R].editor.field.displayField];
						}
						if (K.primaryData[U.name] === undefined) {
							K.primaryData[U.name] = obj[this.grid.colModel.config[R].editor.field.valueField];
						}
					}
					else if (typeof K.data[U.name] == "object")
					{
						var obj = K.data[U.name];
						if (K.displayData[U.name] === undefined) {
							K.displayData[U.name] = obj[this.grid.colModel.config[R].editor.field.displayField];
						}
						if (K.primaryData[U.name] === undefined) {
					...
					if (K.displayData === undefined
							|| K.displayData[U.name] === undefined) {
						displayValue = K.data[U.name];
					} else {
						displayValue = K.displayData[U.name];
					}
				} catch (e) {
				}
				O.value = U.renderer(displayValue, O, K, J, R, M);
				O.style = U.style;
				if (O.value == undefined || O.value === "") {
					O.value = "&#160;"
				}
				if (K.dirty && typeof K.modified[U.name] !== "undefined") {
					if (this.xy_EnableDirty){
	...
	renderUI : function() {
		...
		if (D.enableColumnResize !== false) {
			this.splitone = new Ext.grid.XyGridView.SplitDragZone(D,
					this.mainHd.dom)
		}
		if (D.enableColumnMove) {
			this.columnDrag = new Ext.grid.XyGridView.ColumnDragZone(D,
					this.innerHd);
...
Ext.grid.XyGridView.SplitDragZone = function(A, B) {
	this.grid = A;
	this.view = A.getView();
	this.marker = this.view.resizeMarker;
	this.proxy = this.view.resizeProxy;
	Ext.grid.XyGridView.SplitDragZone.superclass.constructor.call(this, B,
...
Ext.extend(Ext.grid.XyGridView.SplitDragZone, Ext.dd.DDProxy, {
		...
		if (H) {
			...
			if (F !== false) {
				...
				if (this.cm.isResizable(this.cellIndex)
					...
					Ext.grid.XyGridView.SplitDragZone.superclass.handleMouseDown
