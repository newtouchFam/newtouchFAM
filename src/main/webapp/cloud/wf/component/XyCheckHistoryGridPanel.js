Ext.grid.XyCheckHistoryGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent : function() {
		...
		this.store = new Ext.data.JsonStore({
			...
			fields : ["checkid", "checkstep", "activityname", "checkdesc", "displayname", "checktime", "checkdesc" , "isagree"]
		});
		this.store.on("loadexception", showExtLoadException);
		var clnCheckID = {header: "ID", dataIndex: "checkid", hidden: true};
		var clnStep = {header: "步骤", dataIndex: "checkstep", width:65, resizable: false, align : "right"};
		var clnActivity = {header: "环节名称", dataIndex: "activityname", width:180, resizable: true};
		var clnAc = {header: "审批意见", dataIndex: "checkdesc", width:300,renderer:renderDesc};
		var clnUser = {header: "审批者", dataIndex: "displayname", width:80};
		var clnTime = {header: "审批时间", dataIndex: "checktime", width:125, align : "center"};
		var clnType = {header: "类型", dataIndex: "isagree", width:80, sortable: true, renderer: this.onTypeRender, hidden : true };
		this.cm = new Ext.grid.ColumnModel([clnCheckID, clnStep, clnActivity, clnUser,clnTime, clnAc, clnType]);
...
Ext.reg("xycheckhistorygrid", Ext.grid.XyCheckHistoryGridPanel);
