...
bcm.report.BudgetAlter.ConditionWin = Ext.extend(Ext.FormPanel,
{
	...
	initComponent : function()
	{
		...
		this.items.push(this.dtEndDate);
		this.items.push(this.cmbOrderType);
	...
	query : function()
	{
		...
		param.addInteger("budgetyear", this.cmbCase.getDisplayValue());
		...
		var strTitle = this.cmbCase.getDisplayValue() + "年项目预算调整表";

		this.xy_MainPanel.count.call(this.xy_MainPanel,
		{
			jsonCondition : Ext.encode(param)
		}, "SSC/bcm_SpecBudgetAlterAction!count.action", function()
		{
			this.xy_MainPanel.QueryEvent.call(this.xy_MainPanel,
			{
				jasper : "specbudgetalter.jasper",
				subdir : "BCM/core/report/specbudgetalter/",
/*2013-01-29 改在action层处理
				subreportjasper : "BCM/core/report/specbudgetalter/specbudgetalter_sub.jasper",
*/
				title : strTitle,
				fileName : "specbudgetalter",
				basePath : Ext.get("basePath").dom.value,
				jsonCondition : Ext.encode(param)
			}, "SSC/bcm_SpecBudgetAlterAction$report.action");
		}, this);
	}
});
