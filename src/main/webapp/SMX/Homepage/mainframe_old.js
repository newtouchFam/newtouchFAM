...
{
	var passStr = getGUID();
	var root = new Ext.tree.AsyncTreeNode({id:"root", text:"菜单"});
//	var loader = new Ext.tree.TreeLoader({dataUrl:"SMX/GetChildrenMenuAction.action"});
	var loader = new Ext.tree.TreeLoader({dataUrl:"menu/tree"});
	loader.on("loadexception", loadException);
	var tree = new Ext.tree.TreePanel({loader:loader, root:root, animate:true, enableDD:false, border:false, rootVisible:false});
	...
	var funcAreaTabsOpts = {enableTabScroll : true, plain:false, activeTab:0, items :[{title:"欢迎", closable:false, html:welcomeUrl}]};
	...
	var north = {region:"north",height:70,collapsible:true,minSize:70,border:false,html:topUrl};
	...
	var south = {region:"south",height:20,collapsible:true,minSize:20,border:false,html:bottomUrl};
